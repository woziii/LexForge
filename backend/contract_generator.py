...
def _format_cessionnaire(cessionnaire_info):
    ...
    # Pour une personne physique (cas rare mais possible)
    if 'prenom' in cessionnaire_info:
        civility = cessionnaire_info.get('gentille', '')
        first_name = cessionnaire_info.get('prenom', '')
        last_name = cessionnaire_info.get('nom', '')
        address = cessionnaire_info.get('adresse', '')
        return f"{civility} {first_name} {last_name}, domicilié(e) {address}".strip()
    
    # Pour une personne morale (cas le plus courant)
    details = []
    if forme_juridique:
        details.append(forme_juridique)
    if capital:
        details.append(f"au capital de {capital}")
    if rcs:
        details.append(f"immatriculée sous le numéro {rcs}")
    if siege:
        details.append(f"dont le siège social est situé {siege}")
    
    details_text = ", ".join(details)
    
    return f"{nom}, {details_text}" 