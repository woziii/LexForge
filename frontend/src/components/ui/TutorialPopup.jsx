  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black bg-opacity-40">
        <div 
          ref={dialogRef}
          className={`relative max-w-xl w-full mx-2 sm:mx-4 bg-white rounded-xl overflow-hidden transform transition-all duration-500 ${isVisible ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}
          style={{
        ...
        >
          ...
          <div className="px-4 sm:px-6 py-3 sm:py-4 bg-gradient-to-r from-yellow-100 to-yellow-50 border-b border-yellow-200">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm sm:text-base font-semibold text-yellow-800">{tutorialTitle}</h3>
              </div>
              <button
                onClick={onClose}
          ...
          <div className="p-4 sm:p-6 overflow-y-auto" style={{ maxHeight: 'calc(90vh - 56px)' }}>
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6">
              {/* Image de Saul avec effet de carte */}
              <div className="flex flex-col items-center flex-shrink-0 w-24 sm:w-28 bg-yellow-50 p-2 rounded-lg border border-yellow-100 shadow-sm mb-2 sm:mb-0">
                <img 
                  src={messagePath} 
                  alt="Saul" 
              ...
              <div className="flex-grow w-full">
                {/* Bulle de texte */}
                <div 
                  className="bg-yellow-50 px-4 sm:px-5 py-3 sm:py-4 rounded-xl border-2 border-yellow-100 cursor-pointer mb-3 sm:mb-4 shadow-sm"
                ...
                <div className="flex justify-between items-center">
                  <div className="flex space-x-1 sm:space-x-1.5">
                    {messages.map((_, index) => (
                      <div 
                        key={index}
                        className={`h-1.5 sm:h-2 w-4 sm:w-7 rounded-full transition-all duration-300 ${
                  ...
                  <button
                    ...
                    className={`px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-medium rounded-lg transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500 ${
                      isTypingComplete 
